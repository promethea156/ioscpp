...
std::span<const std::byte> Pairing::root_private_key() const noexcept
{
    return impl_->root_private_key;
}
...
Result<TlsSession> TlsSession::start(Stream &stream, const Pairing &pairing)
{
    ...
    mbedtls_ssl_conf_authmode(
        &impl.conf, std::getenv("IOSCPP_NO_VERIFY") != nullptr ? MBEDTLS_SSL_VERIFY_NONE : MBEDTLS_SSL_VERIFY_REQUIRED);
    ...
    // `IOSCPP_TLS12` pins the maximum version to TLS 1.2, for an experiment
    // against a device whose `lockdownd` may not speak TLS 1.3.
    ...
    //   https://github.com/libimobiledevice/libimobiledevice/blob/master/src/idevice.c
    //
    // `IOSCPP_SESSION` echoes the `StartSession` `SessionID` as the TLS session
    // id instead, with the version and suite a bare session is missing.
    if (std::getenv("IOSCPP_SESSION") != nullptr)
    {
        const std::string_view session_id = pairing.session_id();
        mbedtls_ssl_session session;
        mbedtls_ssl_session_init(&session);
        session.MBEDTLS_PRIVATE(tls_version) = MBEDTLS_SSL_VERSION_TLS1_3;
        session.MBEDTLS_PRIVATE(ciphersuite) = 0x1301;
        session.MBEDTLS_PRIVATE(id_len) = std::min(session_id.size(), sizeof(session.MBEDTLS_PRIVATE(id)));
        std::memcpy(session.MBEDTLS_PRIVATE(id), session_id.data(), session.MBEDTLS_PRIVATE(id_len));
        mbedtls_ssl_set_session(&impl.ssl, &session);
