...
device-free over the mock, and its second, the `CoreDeviceProxy` handshake on `Device`, is done and
proven on an iOS 18.7.8 device (`docs/10-coredevice-tunnel.md`). The open work is ordered lowest first:
